...
namespace MyLibrary
{
    ...
    // abstract base class
    [Serializable]
    [XmlInclude(typeof(V2DataOnGrid)), XmlInclude(typeof(V2DataCollection))]
    ...
    public class V2MainCollection : IEnumerable<V2Data>, INotifyCollectionChanged, INotifyPropertyChanged   
    {
        ...
        [XmlArray("V2data_list")]
        [XmlArrayItem("V2DataOnGrid", typeof(V2DataOnGrid))]
        [XmlArrayItem("V2DataCollection", typeof(V2DataCollection))]
        ...
        public void Save(string filename)
        {
            ...
            {
                Type[] types = { typeof(V2Data), typeof(V2DataCollection), typeof(V2DataOnGrid) };
                XmlSerializer serializer = new XmlSerializer(typeof(V2DataCollection), types);
                serializer.Serialize(stream, V2data_list[0]);
            }

            //using (var stream = new FileStream(filename, FileMode.Create))
            //{
            //    Type[] types = { typeof(V2Data), typeof(V2DataCollection), typeof(V2DataOnGrid) };
            //    XmlSerializer serializer = new XmlSerializer(typeof(List<V2Data>), types);
            //    serializer.Serialize(stream, V2data_list);
            //}
